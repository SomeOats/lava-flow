...
export default class LavaFlow {
  ...
  static createUIElements(html: any): void {
    // Guard: don't inject the button more than once. don't present if not GM.
    ...
    const buttonHtml = isV13 
      ...
      : `<div class="${ID}-row action-buttons flexrow"><button class="${className}"><i class="fas fa-upload"></i> ${tooltip}</button></div>`;
